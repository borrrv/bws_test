...
LOG_FILENAME = "bet-maker.log"
LOG_FILENAME_ERR = "bet-maker_error.log"
...
def init_logger():
    ...
    rf_format = logging.Formatter(
        "%(asctime)s:%(name)s:%(filename)s:%(funcName)s:%(levelname)s :%(message)s"
    )
    rf_handler.setFormatter(rf_format)

    # Rotate file handler
    ex_handler = logging.handlers.TimedRotatingFileHandler(
        log_file_error, when="midnight", backupCount=2
    )
    ex_handler.setLevel(logging.ERROR)
    ex_format = logging.Formatter(
        "%(asctime)s:%(name)s:%(filename)s:%(funcName)s:%(levelname)s :%(message)s"
    )
